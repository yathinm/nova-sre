...
.PHONY: cluster-create cluster-delete cluster-info addons addons-ingress dashboard \
        ...
        port-forward-server port-forward-agent port-forward-frontend \
...
port-forward-frontend: ## Forward control panel → localhost:3001
	kubectl port-forward -n nova-sre svc/nova-sre-frontend 3001:3000
